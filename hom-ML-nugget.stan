	...
	vector[,] diffs( matrix X1, matrix X2, int n1, int n2, int K ) {
		...
		for(i in 1:n1){
			for(j in 1:n2){
				tmp1 = (X1[i,]' - X2[j,]');
				D[i,j] = tmp1 .* tmp1;
        //changed from D[i,j] = sqrt(tmp1 .* tmp1);
			}
		}
		...
		if(n1 != n2){
			for(i in 1:n1){
				...
				for(j in 1:n2){ // just compute the lower triangle
					...
					mat[i,j] =  dot_product( omega, tmp1 );
...
Var_Y_c = exp_kern(D_cc, square(c_sigma), exp(-2*log(c_theta)), n_c, n_c, K); //changed from: Var_Y_c = exp_kern(D_cc, square(c_sigma), exp(-log(c_theta)), n_c, n_c, K);
// C(x_c, x_c)

// I can be clever and order the x_c such that the intersection with x_e is at end
Var_Y_e = (rho * rho * Var_Y_c[(1+n_c-n_e):n_c,(1+n_c-n_e):n_c] ) + exp_kern(D_ee, square(m_sigma), exp(-2*log(m_theta)), n_e, n_e, K) +  diag_matrix(rep_vector(m_nugget, n_e)); //changed from Var_Y_e = (rho * rho * Var_Y_c[(1+n_c-n_e):n_c,(1+n_c-n_e):n_c] ) + exp_kern(D_ee, square(m_sigma), exp(-log(m_theta)), n_e, n_e, K) +  diag_matrix(rep_vector(m_nugget, n_e)); 
//Var(Y_e)= rho^2 * part of Var_Y_c corresponding to x_e + C(x_e, x_e) + diag(lambda)

Cov_c_e = rho * exp_kern(D_ce, square(c_sigma), exp(-2*log(c_theta)), n_c, n_e, K)' ; //changed from: Cov_c_e = rho * exp_kern(D_ce, square(c_sigma), exp(-log(c_theta)), n_c, n_e, K)' ;
