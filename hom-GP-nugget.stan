  ...
  vector[,] diffs( matrix X1, matrix X2, int n1, int n2, int K ) {
    ...
    for(i in 1:n1){
      for(j in 1:n2){
        tmp1 = (X1[i,]' - X2[j,]');
        D[i,j] = tmp1 .* tmp1;
        //changed from D[i,j] = sqrt(tmp1 .* tmp1); in expcov version
      }
    }
    ...
    if(n1 != n2){
      for(i in 1:n1){
        ...
        for(j in 1:n2){ // just compute the lower triangle
          ...
          tmp1 = D[i,j];	// K length vector- D[i,j] for each simulator parameter
          
          mat[i,j] =  dot_product( omega, tmp1 );
          //-1 * sum over all k (omega_k * vector of dists btwn X1[i,] and X2[j,] for parameter k)
        }
      }
    }
...
model {
  ...
  //m_var = var_mean + exp_kern(Diffs, square(m_sigma), exp(-2*log(m_theta)), N, N, K); //no nugget
  // m_theta^(-2) = omega
  m_var = var_mean + exp_kern(Diffs, square(m_sigma), exp(-2*log(m_theta)), N, N, K) + diag_matrix(rep_vector(m_nugget, N)); //changed from: m_var = var_mean + exp_kern(Diffs, square(m_sigma), exp(-log(m_theta)), N, N, K) + diag_matrix(rep_vector(m_nugget, N));
